...
<div class="container">
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <form action="{{ route('custom-user.profile.update') }}"
                method="POST">
                @csrf
                @method('PUT')
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <form action="{{ route('custom-user.profile.updatePhoto') }}"
                ...
                <div class="mb-3">
                    <img
                        src="{{ asset('storage/' . $customUser->profile_photo) }}"
                        alt="Foto de Perfil" width="150" class="rounded">
                </div>
                @endif

                {{-- Campo de upload --}}
                <div class="mb-3">
                    <label for="profile_photo" class="form-label">Selecionar
                        nova foto</label>
                    <input type="file" name="profile_photo" id="profile_photo"
                        accept="image/*" class="form-control">
                </div>

                <button type="submit" class="btn btn-secondary">Atualizar
                    Foto</button>
            </form>
        </div>
    </div>
