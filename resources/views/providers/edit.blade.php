...
<div class="container">
    ...
    {{-- Formulário de dados do Provider --}}
    <div class="card mb-4">
            ...
            <form action="{{ route('provider.profile.update') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select name="availability" id="availability" class="form-select" required>
                        <option value="weekdays" {{ old('availability', $provider->availability) === 'weekdays' ? 'selected' : '' }}>Dias úteis</option>
                        <option value="weekends" {{ old('availability', $provider->availability) === 'weekends' ? 'selected' : '' }}>Fins de semana</option>
                        <option value="both" {{ old('availability', $provider->availability) === 'both' ? 'selected' : '' }}>Ambos</option>
                    </select>
                </div>
        ...
        <div class="card-body">
            <form action="{{ route('provider.profile.updatePhoto') }}" method="POST" enctype="multipart/form-data">
                ...
                @if($provider->profile_photo)
                    <div class="mb-3">
                        <img src="{{ asset('storage/' . $provider->profile_photo) }}" alt="Foto de Perfil" width="150" class="rounded">
                    </div>
                @endif
